  ...
  useEffect(() => {
    const buscarProductos = async () => {
      ...
      try {
        setCargando(true);
        setError(null);
        const response = await fetch(`http://localhost:3001/buscar?q=${encodeURIComponent(q.trim())}`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h2 className="text-2xl sm:text-3xl font-semibold text-[#117287]">Resultados para: "{q}"</h2>
        ...
        {productos.map(producto => (
          ...
          >
            ...
            <div className="p-4 flex flex-col flex-1">
              <h3 className="text-lg font-semibold text-gray-800 mb-1 text-center">{producto.name}</h3>
              <p className="text-sm text-gray-600 mb-2 text-center line-clamp-2">{producto.description}</p>
              <p className="text-xl font-bold text-[#117287] text-center mb-4">${parseFloat(producto.price).toFixed(2)}</p>
              <button
